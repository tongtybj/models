...
def dicts_to_tf_example(dicts, root_dir, _set):
  ...
  for data, img_path in zip(dicts, imgs_path):
    ...
    if 'object' in data:
      ...
      print("no object in {}".format(img_path))
...
def main(_):
  ...
  annotations_dir = os.path.join(root_dir, 'Annotations/VID/{}'.format(FLAGS.set))
  ...
  for i in range(start_shard, num_shards):
    ...
    out_filename = os.path.join(FLAGS.output_path, 'VID_2015-'+(shard_format % i)+'.tfrecord')
    ...
    gen_shard(examples_list[start:end], annotations_dir, out_filename, root_dir, FLAGS.set)
